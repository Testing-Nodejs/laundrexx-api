  ...
  try {
    ...
    if (result4.recordsets[0].length > 0) {
      ...
      var result = await pool
        ...
        .query(
          `INSERT INTO SERVICE_CATEGORY(SERVICE_CATEGORY_NAME,SERVICE_CATEGORY_CODE,SERVICE_CATEGORY_HSN,SERVICE_CATEGORY_CGST,SERVICE_CATEGORY_SGST,SERVICE_CATEGORY_ORDER_COUNT) VALUES (@SERVICE_CATEGORY_NAME,@SERVICE_CATEGORY_CODE,@SERVICE_CATEGORY_HSN,@SERVICE_CATEGORY_CGST,@SERVICE_CATEGORY_SGST,@SERVICE_CATEGORY_ORDER_COUNT)`
        );
...
async function UpdateServiceCategory(id, obj) {
  try {
    ...
    var result = await pool
      ...
      .query(
        `update SERVICE_CATEGORY set SERVICE_CATEGORY_ORDER_COUNT = @SERVICE_CATEGORY_ORDER_COUNT, SERVICE_CATEGORY_NAME = @SERVICE_CATEGORY_NAME,SERVICE_CATEGORY_CODE = @SERVICE_CATEGORY_CODE,SERVICE_CATEGORY_HSN = @SERVICE_CATEGORY_HSN,SERVICE_CATEGORY_CGST = @SERVICE_CATEGORY_CGST,SERVICE_CATEGORY_SGST = @SERVICE_CATEGORY_SGST where SERVICE_CATEGORY_PKID = @SERVICE_CATEGORY_PKID`
      );
  ...
  try {
    ...
    if (result4.recordsets[0].length > 0) {
      ...
      var result = await pool
        ...
        .query(
          `INSERT INTO SERVICE_TYPE(SERVICE_TYPE_NAME,SERVICE_TYPE_CODE,SERVICE_TYPE_SURCHARGE) VALUES (@SERVICE_TYPE_NAME,@SERVICE_TYPE_CODE,@SERVICE_TYPE_SURCHARGE)`
        );
      if (result.rowsAffected > 0) {
...
async function UpdateServiceType(id, obj) {
  try {
    ...
    var result = await pool
      ...
      .query(
        `update SERVICE_TYPE set SERVICE_TYPE_NAME = @SERVICE_TYPE_NAME,SERVICE_TYPE_CODE = @SERVICE_TYPE_CODE,SERVICE_TYPE_SURCHARGE=@SERVICE_TYPE_SURCHARGE where SERVICE_TYPE_PKID=@SERVICE_TYPE_PKID`
      );
    if (result.rowsAffected > 0) {
      var result1 = await pool
        .request()
